...
class bangluongController extends Controller
{
    ...
    function tinhluong_khongdinhmuc_moi($inputs){
        $ngaylap = Carbon::create($inputs['nam'], $inputs['thang'], '01');
            ...
            */
            ...
            if(isset($a_pc['pctnn'])){
                switch ($a_pc['pctnn']['phanloai']) {
                    ...
                    case 2: {//phần trăm
                        $heso = 0;
                        foreach (explode(',', $a_pc['pctnn']['congthuc']) as $ct) {
                            if ($ct != '' && $ct != 'pctnn')
                                $heso += $m_cb[$key][$ct];
                        }
                        $m_cb[$key]['pctnn'] = $heso * $m_cb[$key]['pctnn'] / 100;
                        break;
                    }
                    default: {//trường hợp còn lại (ẩn,...)
                        $m_cb[$key]['pctnn'] = 0;
            ...
            foreach ($a_pc as $k=>$v) {
                ...
                if (!$khongluong || !$thaisan
                    ...
                    if ($m_cb[$key][$mapc] > 10000) {//sô tiền
                        $m_cb[$key]['st_'.$mapc] = $daingay ? round($m_cb[$key][$mapc] * $ptdn, 0) : round($m_cb[$key][$mapc], 0);
                        $tien += chkDbl($m_cb[$key][$mapc]);
                    } else {
                        $m_cb[$key]['st_'.$mapc] = $daingay ? round($inputs['luongcoban'] * $m_cb[$key][$mapc] * $ptdn, 0) : round($inputs['luongcoban'] * $m_cb[$key][$mapc], 0);
                        $tonghs += $m_cb[$key][$mapc];
                    }

                    if ($a_pc[$k]['baohiem'] == 1 && !$thaisan && !$daingay) {//nghỉ thai sản + dài ngày ko đóng bảo biểm
                        $a_pc[$k]['stbhxh'] = round($m_cb[$key]['st_'.$mapc] * $m_cb[$key]['bhxh'], 0);
                        $a_pc[$k]['stbhyt'] = round($m_cb[$key]['st_'.$mapc] * $m_cb[$key]['bhyt'], 0);
                        $a_pc[$k]['stkpcd'] = round($m_cb[$key]['st_'.$mapc] * $m_cb[$key]['kpcd'], 0);
                        $a_pc[$k]['stbhtn'] = round($m_cb[$key]['st_'.$mapc] * $m_cb[$key]['bhtn'], 0);
                        $a_pc[$k]['ttbh'] = $a_pc[$k]['stbhxh'] + $a_pc[$k]['stbhyt'] + $a_pc[$k]['stkpcd'] + $a_pc[$k]['stbhtn'];
                        $a_pc[$k]['stbhxh_dv'] = round($m_cb[$key]['st_'.$mapc]['sotien'] * $m_cb[$key]['bhxh_dv'], 0);
                        $a_pc[$k]['stbhyt_dv'] = round($m_cb[$key]['st_'.$mapc]['sotien'] * $m_cb[$key]['bhyt_dv'], 0);
                        $a_pc[$k]['stkpcd_dv'] = round($m_cb[$key]['st_'.$mapc]['sotien'] * $m_cb[$key]['kpcd_dv'], 0);
                        $a_pc[$k]['stbhtn_dv'] = round($m_cb[$key]['st_'.$mapc]['sotien'] * $m_cb[$key]['bhtn_dv'], 0);
        ...
        foreach(array_chunk($a_data_canbo, 50)  as $data){
            $this->storeBangLuong($inputs['thang'],$data);
        }
        ...
        $a_col_cbkn = array('id','bac','bhxh_dv', 'bhtn_dv', 'kpcd_dv', 'bhyt_dv','bhxh','baohiem',
            'bhtn', 'kpcd', 'bhyt', 'macongtac','manguonkp','pthuong','theodoi','phanloai');
