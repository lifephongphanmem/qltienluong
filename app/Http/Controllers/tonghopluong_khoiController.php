...
class tonghopluong_khoiController extends Controller
{
    ...
    function index(Request $requests)
    {
        if (Session::has('admin')) {
            ...
            $a_data = array(
                array('thang' => '01', 'mathdv' => null, 'noidung' => null, 'trangthai' => 'CHUADL', 'sldv' => $this->soluongdv('1', $inputs['nam']), 'dvgui' => 0),
            ...
            */
            ...
            for ($i = 0; $i < count($a_data); $i++) {
                ...
                if ($tonghop != null) { //lấy dữ liệu đã tổng hợp đưa ra kết quả
                    ...
                    } elseif (count($dulieu) + count($dulieu_khoi) == $a_data[$i]['sldv']) { //kiểm tra xem có bao nhiêu đơn vị gửi / tổng số các đơn vị
                        // 2024.11.29 } elseif (count($dulieu) == $a_data[$i]['sldv']) { //kiểm tra xem có bao nhiêu đơn vị gửi / tổng số các đơn vị
        ...
        if (Session::has('admin')) {
            ...
            $model = tonghopluong_donvi_chitiet::join('tonghopluong_donvi', 'tonghopluong_donvi_chitiet.mathdv', 'tonghopluong_donvi.mathdv')
                ...
                ->select(
                    ...
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcct) as pcct'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pckct) as pckct'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pck) as pck'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pccv) as pccv'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pckv) as pckv'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcth) as pcth'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcdd) as pcdd'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcdh) as pcdh'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcld) as pcld'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcdbqh) as pcdbqh'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcudn) as pcudn'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pctn) as pctn'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pctnn) as pctnn'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcdbn) as pcdbn'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcvk) as pcvk'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pckn) as pckn'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcdang) as pcdang'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pccovu) as pccovu'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pclt) as pclt'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcd) as pcd'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pctr) as pctr'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pctdt) as pctdt'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pctnvk) as pctnvk'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcbdhdcu) as pcbdhdcu'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.pcthni) as pcthni'),
                    'tonghopluong_donvi_chitiet.tonghs',
                    'tonghopluong_donvi_chitiet.giaml',
                    DB::raw('SUM(tonghopluong_donvi_chitiet.luongtn) as luongtn'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.stbhxh_dv) as stbhxh_dv'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.stbhyt_dv) as stbhyt_dv'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.stkpcd_dv) as stkpcd_dv'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.stbhtn_dv) as stbhtn_dv'),
                    DB::raw('SUM(tonghopluong_donvi_chitiet.ttbh_dv) as ttbh_dv')
                )
