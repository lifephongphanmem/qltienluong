    ...
    <table id="data_header" class="header" width="96%" border="0" cellspacing="0" cellpadding="8"
        ...
        <tr>
            <td colspan="2" style="text-align: center; font-weight: bold; font-size: 20px; text-transform: uppercase">
                BẢNG TỔNG HỢP HỆ SỐ TIỀN LƯƠNG VÀ PHỤ CẤP CỦA ĐẠI BIỂU HĐND, CẤP UỶ VIÊN NĂM {{ $inputs['namns'] }}
            </td>
        </tr>
    ...
    <table id="data_body" cellspacing="0" cellpadding="0" border="1"
        ...
        <thead>
            ...
            <tr style="padding-left: 2px;padding-right: 2px">
                <th rowspan="2" style="width: 3%;">ĐƯỢC<br>GIAO</th>
                <th rowspan="2" style="width: 3%;">CÓ<br>MẶT</th>
        ...
        <tr style="font-weight: bold; text-align: center;">
            <td></td>
            <td>TỔNG CỘNG</td>
            <td class="text-center">{{ dinhdangso($model->sum('canbo_dutoan')) }}</td>
        ...
        @foreach ($m_phanloai->where('capdo_nhom', '1')->sortby('sapxep') as $phanloai1)
            ...
            <tr class="font-weight-bold">
                ...
                <td class="text-center">{{ dinhdangso($model_chitiet->sum('canbo_dutoan')) }}</td>
            ...
            @foreach ($a_phanloaicongtac as $phanloaicongtac)
                ...
                <tr>
                    <td></td>
                    <td>{{ $a_plct[$phanloaicongtac] ?? $phanloaicongtac }}</td>
                    <td class="text-center">{{ dinhdangso($chitiet->sum('canbo_dutoan')) }}</td>
                    <td class="text-center">{{ dinhdangso($chitiet->sum('canbo_congtac')) }}</td>
            ...
            @if (count($model_donvi) > 0)
                ...
                @if ($phanloai1->chitiet == '1')
                    @foreach ($model_donvi as $donvi)
                        ...
                        if ($model_chitiet->sum('quyluong') <= 0) {
                            continue;
                        }
                        ?>
                        <tr class="font-weight-bold">
                ...
                @else
                    ...
                    {{-- model_chitiet => tạo mảng theo phân loại công tác => dai dữ liệu theo phân loại công tác --}}
                @endif
            @endif
            {{-- vòng 2 --}}
