    ...
    <table id='data_body' class="money" cellspacing="0" cellpadding="0" border="1" style="margin: 20px auto; border-collapse: collapse;font:normal 12px Times, serif;">
        ...
        @for($j=0;$j<count($a_nguon);$j++)
            ...
            <?php
                ...
                $model_dv = a_getelement($a_dv,$a_nguon[$j]);
            ?>
            ...
            @foreach($model_congtac as $congtac)
                <?php
                    ...
                    $donvi = a_getelement($model_dv,$congtac);
                ?>
                @if(count($model_luong)> 0)
                    ...
                    @foreach($donvi as $dv)
                        ...
                        <?php
                            $chitiet = $model_luong->where('madv',$dv['madv']);
                            ?>
                        ...
                        @endforeach
                            <tr style="font-weight: bold; text-align: center; font-style: italic">
                                <td colspan="3">Cộng {{$dv['tendv']}}</td>
                                @foreach($a_phucap as $key=>$val)
                                    <td>{{dinhdangsothapphan($chitiet->sum($key) ,5)}}</td>
                                @endforeach
                                <td>{{dinhdangsothapphan($chitiet->sum('tonghs') ,5)}}</td>

                                <td class="money">{{dinhdangso($chitiet->sum('tongtl'))}}</td>
                                <td class="money">{{dinhdangso($chitiet->sum('giaml'))}}</td>
                                <td class="money">{{dinhdangso($chitiet->sum('tongtl') - $chitiet->sum('giaml'))}}</td>

                                <td class="money">{{dinhdangso($chitiet->sum('stbhxh_dv'))}}</td>
                                <td class="money">{{dinhdangso($chitiet->sum('stbhyt_dv'))}}</td>
                                <td class="money">{{dinhdangso($chitiet->sum('stkpcd_dv'))}}</td>
                                <td class="money">{{dinhdangso($chitiet->sum('stbhtn_dv'))}}</td>
                                <td class="money">{{dinhdangso($chitiet->sum('ttbh_dv'))}}</td>
                                <td></td>
                            </tr>
                    @endforeach
