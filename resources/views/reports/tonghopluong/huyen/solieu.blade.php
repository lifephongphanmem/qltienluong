...
@section('content')
    ...
    <table id="data_body" class="header" width="96%" border="0" cellspacing="0" cellpadding="8"
        ...
        <tr>
            <td colspan="{{$col + 13}}">
                <p style="text-align: center; font-weight: bold; font-size: 20px;">SỐ LIỆU TỔNG HỢP CHI TRẢ LƯƠNG TẠI ĐƠN VỊ
                    CẤP DƯỚI</p>
            </td>
        ...
        <tr>
            <td colspan="{{$col + 13}}">
                <p  style="text-align: center; font-style: italic">Tháng {{ $thongtin['thang'] }} năm
                    {{ $thongtin['nam'] }}</p>
            </td>
    ...
    <table id="data_body1" cellspacing="0" cellpadding="0" border="1"
        ...
        @foreach ($model_phanloai as $pl)
            ...
            @foreach ($donvi as $dv)
                ...
                <?php
                $stt = 1;
                $a_dv = a_getelement($a_soluong, ['maphanloai' => $pl->maphanloai, 'madv' => $dv->madv]);
                ...
                @if (count($chitiet) > 0)
                    <tr class="money" style="font-weight: bold; font-style: italic">
                        ...
                        <td>{{ dinhdangso($chitiet->sum('tongbh') + $chitiet->sum('luongtn') - $chitiet->sum('giaml')) }}
                        </td>
                    </tr>
                @endif
            @endforeach
            @if (count($phanloai) > 0)
                <tr class="money" style="font-weight: bold">
                    ...
                    <td>{{ dinhdangso($phanloai->sum('tongbh') + $phanloai->sum('luongtn') - $phanloai->sum('giaml')) }}
                    </td>
                </tr>
            @endif
        @endforeach
