...
@section('content')
    ...
    <table id="data_body" class="money" cellspacing="0" cellpadding="0" border="1" style="margin: 20px auto; border-collapse: collapse;font:normal 12px Times, serif;">
        <tr style="padding-left: 2px;padding-right: 2px">
            ...
            <th colspan="6">Các khoản trích nộp theo lương</th>
        ...
        @for($j=0;$j<count($a_nguon);$j++)
            ...
            <tr style="font-weight: bold;">
                <td></td>
                <td style="text-align: left;" colspan="{{14 + $col}}">{{$a_nguon[$j]['tennguonkp']}}</td>
            </tr>
                ...
                @if(count($model_luong)> 0)
                    ...
                    @foreach($model_luong as $ct)
                        <tr>
                            ...
                            <td>{{dinhdangsothapphan($ct->stbhxh_dv,5)}}</td>
                            <td>{{dinhdangsothapphan($ct->stbhyt_dv,5)}}</td>
                            <td>{{dinhdangsothapphan($ct->stkpcd_dv,5)}}</td>
                            <td>{{dinhdangsothapphan($ct->stbhtn_dv,5)}}</td>

                            <td>{{dinhdangsothapphan($ct->ttbh_dv,5)}}</td>
                            <td>{{dinhdangso($ct->stbh_dv)}}</td>
                            <td>{{dinhdangso($ct->stbh_dv+$ct->tongtl - $ct->giaml)}}</td>
                            <td></td>
                        </tr>
                    @endforeach
                    ...
                    <tr style="font-weight: bold; text-align: center; font-style: italic">
                        ...
                        <td class="money">{{dinhdangsothapphan($model_luong->sum('stbhxh_dv'),5)}}</td>
                        <td class="money">{{dinhdangsothapphan($model_luong->sum('stbhyt_dv'),5)}}</td>
                        <td class="money">{{dinhdangsothapphan($model_luong->sum('stkpcd_dv'),5)}}</td>
                        <td class="money">{{dinhdangsothapphan($model_luong->sum('stbhtn_dv'),5)}}</td>
                        <td class="money">{{dinhdangsothapphan($model_luong->sum('ttbh_dv'),5)}}</td>
                        <td class="money">{{dinhdangso($model_luong->sum('stbh_dv'))}}</td>
                        <td class="money">{{dinhdangso($model_luong->sum('stbh_dv') + $model_luong->sum('tongtl') - $model_luong->sum('giaml'))}}</td>
                        <td></td>
                    </tr>
                @endif
            @endforeach
        ...
        <tr style="font-weight: bold; text-align: center;">
            ...
            <td class="money">{{dinhdangsothapphan($model->sum('stbhxh_dv'),5)}}</td>
            <td class="money">{{dinhdangsothapphan($model->sum('stbhyt_dv'),5)}}</td>
            <td class="money">{{dinhdangsothapphan($model->sum('stkpcd_dv'),5)}}</td>
            <td class="money">{{dinhdangsothapphan($model->sum('stbhtn_dv'),5)}}</td>
            <td class="money">{{dinhdangsothapphan($model->sum('ttbh_dv'),5)}}</td>
            <td class="money">{{dinhdangso($model->sum('stbh_dv'))}}</td>
            <td class="money">{{dinhdangso($model->sum('stbh_dv') + $model->sum('tongtl') - $model->sum('giaml'))}}</td>
            <td></td>
        </tr>
    </table>
