...
<table id="data_body3" cellspacing="0" cellpadding="0" border="1" style="margin: 20px auto; border-collapse: collapse;">
    ...
    <tr style="font-weight: bold">
        ...
        <th style="text-align: center">{{dinhdangso($a_It['tongsonguoi'])}}</th>
        <th style="text-align: right">{{dinhdangso($a_It['quy1'])}}</th>
        <th style="text-align: right">{{dinhdangso($a_It['quy2'])}}</th>
        <th style="text-align: right">{{dinhdangso($a_It['quy1thangtangthem'])}}</th>
        <th style="text-align: right">{{dinhdangso($a_It['bhyttangthem'])}}</th>
        <th style="text-align: right">{{dinhdangso($a_It['tong'])}}</th>
    </tr>


<?php $i=1 ?>
    @foreach($ar_I as $key=>$dulieu)
    <?php $k=++$key?>
        <tr style="text-align: right">
            <td style="text-align: center">{{$dulieu['tt']}}</td>
            <td style="text-align: left">{{$dulieu['noidung']}}</td>
            <td style="text-align: center">{{dinhdangso($dulieu['tongsonguoi'.$k])}}</td>
            <td>{{dinhdangso($dulieu['quy1_'.$k])}}</td>
            <td>{{dinhdangso($dulieu['quy2_'.$k])}}</td>
            <td>{{dinhdangso($dulieu['quy1thangtangthem'])}}</td>
            <td>{{dinhdangso($dulieu['bhyttangthem'])}}</td>
