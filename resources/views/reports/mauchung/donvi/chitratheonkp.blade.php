...
<p id="data_body" style="text-align: center; font-weight: bold; font-size: 20px;">TỔNG HỢP CHI TRẢ LƯƠNG TẠI ĐƠN VỊ THEO NGUỒN KINH PHÍ</p>
...
<table id="data_body3" cellspacing="0" cellpadding="0" border="1" style="margin: 20px auto; border-collapse: collapse;">
    <thead>
        <tr style="">
            ...
            <th style="width: 15%;" rowspan="2">Nguồn kinh phí</th>
    ...
    @foreach($model as $tonghop)
        ...
        <tr style="font-weight: bold; text-align: center" class="money">
            ...
            <td>{{dinhdangso($chitiet->sum('ttbh_dv'))}}</td>
            <td>{{dinhdangso($chitiet->sum('tongcong'))}}</td>
        </tr>
    @endforeach
    <tr style="font-weight: bold; text-align: center" class="money">
        ...
        @endforeach
        <td>{{dinhdangso($model_chitiet->sum('ttl'))}}</td>
        <td>{{dinhdangso($model_chitiet->sum('stbhxh_dv'))}}</td>
        <td>{{dinhdangso($model_chitiet->sum('stbhyt_dv'))}}</td>
        <td>{{dinhdangso($model_chitiet->sum('stkpcd_dv'))}}</td>
        <td>{{dinhdangso($model_chitiet->sum('stbhtn_dv'))}}</td>
        <td>{{dinhdangso($model_chitiet->sum('ttbh_dv'))}}</td>
        <td>{{dinhdangso($model_chitiet->sum('tongcong'))}}</td>
    </tr>
</table>
