    ...
    <table id="data_body" class="money" cellspacing="0" cellpadding="0" border="1" style="margin: 20px auto; border-collapse: collapse;font:normal 12px Times, serif;">
        ...
        @for($j=0;$j<count($model_thang);$j++)
            <?php
                ...
                $a_thang = $model->where('mabl',$model_thang[$j]['mabl']);
            ?>
            ...
            @foreach($a_ct as $key=>$val)
                <?php
                    $model_luong = $model->where('mact',$key)->where('mabl',$model_thang[$j]['mabl']);
                ?>
            ...
            <tr style="font-weight: bold; text-align: center">
                ...
                <td class="money">{{dinhdangso($a_thang->sum('luongtn') + $a_thang->sum('ttbh_dv'))}}</td>
                <td></td>
            </tr>
        @endfor
        <tr style="font-weight: bold; text-align: center;">
            ...
            <td class="money">{{dinhdangso($model->sum('luongtn') + $model->sum('ttbh_dv'))}}</td>
            <td></td>
        </tr>
    </table>
