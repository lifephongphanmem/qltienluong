...
@section('content')
    ...
    <table id="data_body" class="money" cellspacing="0" cellpadding="0" border="1" style="margin: 20px auto; border-collapse: collapse;font:normal {{$thongtin['cochu']}}px Times, serif;">
        ...
        <?php $i=1; ?>
        @for($i_ct=0;$i_ct<count($a_phongban);$i_ct++)
            <?php $a_luong = a_getelement_equal($a_canbo, array('mapb'=>$a_phongban[$i_ct]['mapb'])) ?>
            @if(count($a_luong)> 0)
                <?php $stt=1; ?>
                <tr style="font-weight: bold;">
                    <td>{{convert2Roman($i++)}}</td>
                    <td style="text-align: left;" colspan="{{19 + $col}}">{{isset($a_pb[$a_phongban[$i_ct]['mapb']])? $a_pb[$a_phongban[$i_ct]['mapb']] : ''}}</td>
                </tr>
