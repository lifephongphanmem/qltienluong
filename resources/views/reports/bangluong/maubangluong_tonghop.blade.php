...
<table id="data_body2" cellspacing="0" cellpadding="0" border="1" style="margin: 20px auto; border-collapse: collapse;">
    ...
    @foreach($model_kpb as $pb)
        ...
        <tr style="font-weight: bold; text-align: center">
            <td style="text-align: left" colspan="17">{{$pb->tenkhoipb}}</td>
        </tr>
        <?php $i=1; ?>
        @foreach($donvi as $dv)
            <?php $model=$model_bangluong_ct->where('mabl',$dv->mabl); ?>
                <tr style="font-weight: bold; text-align: center">
                    <td>{{$i++}}</td>
                    <td style="text-align: left" colspan="16">{{$dv->tendv}}</td>
                </tr>
            <?php $stt=1; ?>
            @foreach($model as $ct)
                <tr>
    ...
    <tr style="font-weight: bold; text-align: center">
        ...
        <td></td>
        <td></td>
        <td>{{number_format($model_bangluong_ct->sum('ttl'))}}</td>
        <td></td>
        <td></td>
        <td>{{number_format($model_bangluong_ct->sum('ttl'))}}</td>
        <td>{{number_format($model_bangluong_ct->sum('bhct'))}}</td>
        <td>{{number_format($model_bangluong_ct->sum('stbhxh'))}}</td>
        <td>{{number_format($model_bangluong_ct->sum('stbhyt'))}}</td>
        <td>{{number_format($model_bangluong_ct->sum('stkpcd'))}}</td>
        <td>{{number_format($model_bangluong_ct->sum('ttbh'))}}</td>
        <td>{{number_format($model_bangluong_ct->sum('luongtn'))}}</td>
        <td></td>
    </tr>
</table>
