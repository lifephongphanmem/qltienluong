...
@section('content')
        ...
        <div class="col-md-12">
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-list-alt"></i>DANH SÁCH PHÂN BỔ LƯƠNG CỦA ĐƠN VỊ
                    </div>
                    <div class="actions">
                        <button type="button" id="_btnadd" class="btn btn-default btn-xs" onclick="add()"><i
                                class="fa fa-plus"></i>&nbsp;Thêm mới phân bổ lương</button>
                    </div>
                </div>
                            ...
                            @if (isset($model))
                                @foreach ($model as $key => $value)
                                    <tr class="{{ getTextStatus($value['trangthai']) }}">
                                        ...
                                        <td class="text-right">{{ dinhdangsothapphan($value->luongnb_pbl) }}</td>
                                        <td class="text-right">{{ dinhdangsothapphan($value->luonghs_pbl) }}</td>
                                        <td class="text-right">{{ dinhdangsothapphan($value->luongbh_pbl) }}</td>
    ...
    <!--Modal thêm mới -->
    {!! Form::open(['url' => $furl.'/thongtin', 'id' => 'frm_dutoan', 'class' => 'form-horizontal']) !!}
    <div id="create-modal" tabindex="-1" role="dialog" aria-hidden="true" class="modal fade">
        <div class="modal-dialog modal-lg modal-content">
            <div class="modal-header modal-header-primary">
                <button type="button" data-dismiss="modal" aria-hidden="true" class="close">&times;</button>
                <h4 id="modal-header-primary-label" class="modal-title">Thông tin phân bổ lương</h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <label><b>Phần mềm sẽ tổng hợp cán bộ ở 02 bảng lương để tạo phân bổ (ưu tiên cán bộ ở bảng lương
                                thứ nhất).</b></label>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-3">
                        <label class="control-label">Năm tạo phân bổ lương</label>
                        {!! Form::text('namns', date('Y'), ['id' => 'namns', 'class' => 'form-control text-right']) !!}
                    </div>
